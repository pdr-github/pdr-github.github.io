---
layout: artpost
title: vEGU21
---

<style>
	#vegu21 {
		background-color: black;
		color: white}
</style>
<div>
  <div class="row">
    <div class="column">
      <img src="/Portfolio/Commissions/egu21_mountains.jpg" alt="Mountains" style="width:100%" onclick="onClick(this)" class="hover-shadow cursor">
    </div>
    <div class="column">
      <img src="/Portfolio/Commissions/egu21_chime.jpg" alt="Mountains" style="width:100%" onclick="onClick(this)" class="hover-shadow cursor">
    </div>
    <div class="column">
      <img src="/Portfolio/Commissions/egu21_aq-india" alt="Mountains" style="width:100%" onclick="onClick(this)" class="hover-shadow cursor">
    </div>
    <div class="column">
      <img src="/Portfolio/Commissions/egu21_aq-africa.jpg" alt="Mountains" style="width:100%" onclick="onClick(this)" class="hover-shadow cursor">
    </div>
  </div>
  {% include Scripts/simplemodal.html %}
</div>

